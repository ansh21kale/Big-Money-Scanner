...
class ContractState:
    ...
    volume_delta_samples: deque = field(
        default_factory=lambda:
        deque(maxlen=60)
    )
    ...
    def update(
        ...
        *,
        ...
        self.volume_delta_samples.append(
            volume_change
        )
    ...
    @property
    def oi_change_60s(self):
        if len(self.oi_history) < 2:
            return 0.0

        oldest_oi = self.oi_history[0][1]
        return self.oi - oldest_oi

    @property
    def oi_change_60s_pct(self):
        if len(self.oi_history) < 2:
            return 0.0
    ...
    @property
    def money_action(self):
        price = self.price_change_60s_pct
        oi = self.oi_change_60s

        if price > 0.15 and oi > 0:
            return "BUY ENTRY"
    ...
    @property
    def event_type(self):
        if "ENTRY" in self.money_action:
            return "ENTRY"

        if "EXIT" in self.money_action:
            return "EXIT"
